...
// Setup microphone and start recognition
setupMicrophone().then(() => {
    checkVoiceVolume(); // Start monitoring volume
...
function loadMediaFile(file) {
	...
	readAudioMetadata(file, function(hasArtwork) {
		if (hasArtwork) {
			...
			const selectedLogo = localStorage.getItem('selectedLogo');
			if (!selectedLogo && defaultLogoLinks.length > 0) {
				const firstLogoSrc = defaultLogoLinks[0].getAttribute('data-src');
				setSelectedLogo(firstLogoSrc); // Automatically set and display the first logo
			} else {
				// If a logo is already selected, display it
				setSelectedLogo(selectedLogo);
			}
		}
	});
